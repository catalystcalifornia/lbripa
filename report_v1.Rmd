...
df_tot<-df_tot%>%
    mutate(label_simple=ifelse(label %in% "22350 VC - UNSAFE SPEED:PREVAIL COND (I) 54106", "Unsafe speed for prevailing conditions (speeding)",
                     ifelse(label %in% "4000(A)(1) VC - NO REG:VEH/TRAILER/ETC (I) 54657", "No vehicle or trailer registration",
                            ifelse(label %in% "22450(A) VC - FAIL STOP VEH:XWALK/ETC (I) 54167", "Failure to stop at a stop sign",
                                   ifelse(label %in% "21453(A) VC - FAIL STOP LINE/ETC AT RED (I) 54098", "Failure to stop at a red light",
                                           ifelse(label %in% "21461(A) VC - DRIVER FAIL OBEY SIGN/ETC (I) 54146", "Driver failure to obey signs/signals", 'NULL'
                     ))))))
...
ft <-   ft%>%set_caption(
    as_paragraph(
      as_chunk("Top Five Traffic Citations Given by LBPD",
               props = fp_text_default(font.family = main_font ),align="left")
    ), word_stylename = "Table Caption",
    fp_p = fpp)%>%
  add_footer_lines(paste0(sourcenote))
