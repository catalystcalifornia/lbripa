...
fx_lollichart(
  ...
  tooltip_text=list(headerFormat="",pointFormat = "<b>{point.hours_rate:.1f}%</b> of hours are spent on <b>{point.stop_reason_short}</b>, or a total of {point.hours_count:,.1f} hours"), # customize your tooltip as a statement, calling out rate and count
...
fx_lollichart(
  ...
  tooltip_text=list(headerFormat="",pointFormat = "<b>{point.stop_denom_rate:,.1f}%</b> of stops of <b>{point.race_label_short}</b> people result in No Action, or a total of {point.stop_count:,.1f} stops"), # customize your tooltip as a statement, calling out rate and count

  chart_caption = paste0(racenote,"<br>", sourcenote), # if necessary add a method note between the racenote and sourcenote with a line break, use racenote if graph includes data by race, and always use sourcenote. Each of these are standard objects across all charts
  yaxis_label = "'%'", #format for your y axis labels
  export_data_label=list(pointFormat='{point.stop_denom_rate:.1f}%')) # format for your data labels on export images
...
lime_green <- c('#2BC433')
persian_indigo <- c('#362178')
halloween_orange <- c('#F25922')
bitter_lemon <- c('#BCD313')
blue <- c('#177FEB')
dark_tangerine <- c('#FF9E0D')
halaya_ube <- c('#733256')
...
addCircleMarkers(data = nh_black_dots, group = "Black", label = NULL, weight = 2, fillOpacity = 1, radius = 2, color="white",  fillColor=blue, stroke = TRUE) %>% 
 ...
 addCircleMarkers(data = mesa_dots, group = "MESA", label = NULL, weight = 2, fillOpacity = 1, radius = 2, color="white",  fillColor= dark_silver, stroke = TRUE) %>% 
 ...
 addCircleMarkers(data = nhpi_dots, group = "NHPI", label = NULL, weight = 2, fillOpacity = 1, radius = 2, color="white",  fillColor= halaya_ube, stroke = TRUE) %>% 
  ...
  addLayersControl(overlayGroups = c("Black", "Latinx", "Asian", "White",  "MESA", "NHPI", "AIAN", "Two or More Races"),
                   options = layersControlOptions(collapsed = FALSE))  %>%
  ...
  ## add Legend
   ...
   addLegend(position = "bottomleft",   title = "Each dot represents 10 officer-initiated stops by race/ethnicity <br> Observations by race may overlap", colors = NA, labels = "Note: Each dot does not represent the actual location of a stop.
            <br> The map summarizes the distribution of LBPD officer-initiated stops <br> by council district and race.") %>%
  
  
  
  addLegend(position = "bottomleft",   title = "", group = "Black", labels = c("Black stop count"), colors = colors_function(colors = blue, sizes = "10", shapes = "circle", borders = NULL) ) %>%
  
    addLegend(position = "bottomleft",   title = "", group = "White", labels = c("White stop count"), colors = colors_function(colors =  bitter_lemon, sizes = "10", shapes = "circle", borders = NULL) ) %>%
  
   addLegend(position = "bottomleft",   title = "", group = "AIAN", labels = c("AIAN stop count"), colors = colors_function(colors = dark_tangerine, sizes = "10", shapes = "circle", borders = NULL) ) %>%
  
   addLegend(position = "bottomleft",   title = "", group = "Latinx", labels = c("Latinx stop count"), colors = colors_function(colors = lime_green, sizes = "10", shapes = "circle", borders = NULL) ) %>%
  
   addLegend(position = "bottomleft",   title = "", group = "MESA", labels = c("MESA stop count"), colors = colors_function(colors = dark_silver, sizes = "10", shapes = "circle", borders = NULL) ) %>%
  
   addLegend(position = "bottomleft",   title = "", group = "Asian", labels = c("Asian stop count"), colors = colors_function(colors = halloween_orange, sizes = "10", shapes = "circle", borders = NULL) ) %>%
  
   addLegend(position = "bottomleft",   title = "", group = "Two or More Races", labels = c("Two or More Races stop count"), colors = colors_function(colors =persian_indigo, sizes = "10", shapes = "circle", borders = NULL) ) %>%
  
   addLegend(position = "bottomleft",   title = "", group = "NHPI", labels = c("NHPI stop count"), colors = colors_function(colors =halaya_ube, sizes = "10", shapes = "circle", borders = NULL) ) %>%
  ...
  addProviderTiles("CartoDB.Positron")%>%
  setView(-118.1888, 	33.77091	, zoom = 11) %>% #base and view 
...
label = paste0(
 ...
 council_district_noaction$council_num, "</span> (Rep. <span style='font-size: 12px; font-weight: bold;'>",council_district_noaction$representat,"</span>), the total rate of LBPD Hours spent on officer-initiated stops that resulted in no action is <span style='font-size: 12px; font-weight: bold;'>",  
 scales::comma(council_district_noaction$hours_rate_noaction_cap, accuracy = 0.1), "</span>%, a total of <span style='font-size: 14px; font-weight: bold;'> ", scales::comma(council_district_noaction$hours_count_noaction_cap, accuracy = 0.1), "</span> hours.")))




#palette purples
pal_stop_hours_rate_noaction_cap <- colorQuantile("Blues",council_district_noaction$hours_rate_noaction_cap, n = 5)


leaflet(width = "100%", height = "495px") %>%
  # add Long beach district layer with stop rates
  addPolygons(data =  council_district_noaction,  label=~htmlEscape(paste0("Council Number ", council_num)), color = "#444444", weight = 2, smoothFactor = 0.5, opacity = 1, fillOpacity = 2, popup = ~paste0(hours_rate_noaction_cap_popup),
  ...
  addLegend(position = "bottomleft", pal = pal_stop_hours_rate_noaction_cap, values = council_district_noaction$hours_rate_noaction_cap, opacity = .5, title = "Percent of hours spent on officer initiated stops that result in no action out of all total hours spent on officer-initiated stops", na.label = NA, labFormat = function(type, cuts, p) {
    n = length(cuts)
...
leaflet(width = "100%", height = "495px") %>%
  ...
  addPolylines(data = high_injury_motor_corridors_line_sf, group = "High Injury Motor Intersections/Corridors",
              ...
              color = "yellow", weight = 4, opacity = 2) %>%
...
fx_lollichart(
  ...
  top_finding=paste0("The majority of searches conducted during traffic stops lead to no evidence or contraband found."), # this is the top finding from the chart that gets used like a title
  ...
  tooltip_text=list(headerFormat="",pointFormat = "For every 100 searches done during traffic stops of people identified as <b>{point.race_label_long:.1f}</b>, LBPD did not find contraband/evidence in <b>{point.hit_rates:.1f}</b>% of those stops,  a total of <b>{point.evidence_not_found:.1f}</b> unnecessary searches."), # customize your tooltip as a statement, calling out rate and count
