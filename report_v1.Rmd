...
report_stoprates_race_person$stop_count <- as.character(formattable::comma(report_stoprates_race_person$stop_count,0))

# Apply Chart

fx_lollichart(

df=report_stoprates_race_person, #name of dataframe
...
tooltip_text=list(headerFormat="",pointFormat = "For every 1K <b>{point.race_label_long:.1f}</b> people in Long Beach, LBPD stopped <b>{point.stop_rate:.1f}</b> people identified as <b>{point.race_label_long:.1f}</b>, a total of <b>{point.stop_count:.1f}</b> people."), # customize your tooltip as a statement, calling out rate and count
...
col <-c(meteorite, lavender, orange, peridot, "#BE3075", "#EB001F", "#64A12D", "#FFED00",
          "#000000")

#### Item Chart 1: Traffic Stops + Results -total hours ####
...
hchart(
  ...
  hc_add_theme(cc_theme)%>%
    hc_exporting(
    enabled = TRUE, sourceWidth=900, sourceHeight=600, 
    chartOptions=list(plotOptions=list(series=list(dataLabels=list(enabled=TRUE, format=paste0(list(pointFormat='{point.hours_count:.1f}')))))),
  ...
  hc_legend(title=list(text="Result of Stop"), 
              enable = TRUE, labelFormat = '{name} <span style="opacity: 0.4">{hours_count:.1f}</span>')



#### Item Chart 2: Traffic Stops + Results -rate of hours (/100) ####
...
leaflet(width = "100%", height = "495px") %>%
   ...
   # add Long beach district layer with stop rates
  addPolygons(data = council_district_rate, label=~htmlEscape(paste0("Council Number ", council_num)), color = "black", opacity = 1, fill=F,  weight = 2, smoothFactor = 0.5,  highlight = highlightOptions(color = "white", weight = 7, bringToFront = FALSE, sendToBack = FALSE)) %>%

  
  ## add NH Black Stops
  
addCircleMarkers(data = nh_black_dots, popup = ~paste0(" <span style='font-size: 14px; font-weight: bold;'> 10 <span style='font-size: 14px; font-weight: bold;'>", race_label_long, " Drivers Stopped by LBPD"), group = "Black", label = NULL, weight = 2, fillOpacity = 1, radius = 3.5, color="white",  fillColor= "#332985", stroke = TRUE) %>% 

    ## add NH White Stops
  
addCircleMarkers(data = nh_white_dots,popup = ~paste0(" <span style='font-size: 14px; font-weight: bold;'> 10 <span style='font-size: 14px; font-weight: bold;'>", race_label_long, " Drivers Stopped by LBPD"), group = "White", label = NULL, weight = 2, fillOpacity = 1, radius = 3.5, color="white",  fillColor= "#332985", stroke = TRUE) %>% 
  
  
### add AIAN dots 
  
  addCircleMarkers(data = aian_dots, popup = ~paste0(" <span style='font-size: 14px; font-weight: bold;'> 10 <span style='font-size: 14px; font-weight: bold;'>", race_label_long, " Drivers Stopped by LBPD"), group = "AIAN", label = NULL, weight = 2, fillOpacity = 1, radius = 3.5, color="white",  fillColor= "#332985", stroke = TRUE) %>% 
  
### add latinx dots
  
  addCircleMarkers(data = latinx_dots, popup = ~paste0(" <span style='font-size: 14px; font-weight: bold;'> 10 <span style='font-size: 14px; font-weight: bold;'>", race_label_long, " Drivers Stopped by LBPD"), group = "Latinx", label = NULL, weight = 2, fillOpacity = 1, radius = 3.5, color="white",  fillColor= "#332985", stroke = TRUE) %>% 

  ### add MESA dots
  
  addCircleMarkers(data = mesa_dots, popup = ~paste0(" <span style='font-size: 14px; font-weight: bold;'> 10 <span style='font-size: 14px; font-weight: bold;'>", race_label_long, " Drivers Stopped by LBPD"), group = "MESA", label = NULL, weight = 2, fillOpacity = 1, radius = 3.5, color="white",  fillColor= "#332985", stroke = TRUE) %>% 
  
  ### add nh asian dots
  
  addCircleMarkers(data = nh_asian_dots, popup = ~paste0(" <span style='font-size: 14px; font-weight: bold;'> 10 <span style='font-size: 14px; font-weight: bold;'>", race_label_long, " Drivers Stopped by LBPD"), group = "Asian", label = NULL, weight = 2, fillOpacity = 1, radius = 3.5, color="white",  fillColor= "#332985", stroke = TRUE) %>% 
  
  ### add nh two or more dots

  addCircleMarkers(data = nh_twoormor_dots, popup = ~paste0(" <span style='font-size: 14px; font-weight: bold;'> 10 <span style='font-size: 14px; font-weight: bold;'>", race_label_long, " Drivers Stopped by LBPD"), group = "Two or More Races", label = NULL, weight = 2, fillOpacity = 1, radius = 3.5, color="white",  fillColor= "#332985", stroke = TRUE) %>% 
  
  ### add nhpi dots
  
   addCircleMarkers(data = nhpi_dots, popup = ~paste0(" <span style='font-size: 14px; font-weight: bold;'> 10 <span style='font-size: 14px; font-weight: bold;'>", race_label_long, " Drivers Stopped by LBPD"), group = "NHPI", label = NULL, weight = 2, fillOpacity = 1, radius = 3.5, color="white",  fillColor= "#332985", stroke = TRUE) %>% 
  
  addLayersControl(baseGroups = c("Black", "White", "AIAN", "Latinx", "MESA","Asian", "Two or More Races", "NHPI"),
                   options = layersControlOptions(collapsed = FALSE))  %>%
  
  ## add Legend
  
 addLegend(position = "bottomleft",   title = "LBPD Traffic Stops", labels = c("Each Dot Represents 10 Traffic Officer-Initiated Stops by Race/Ethnicity"), colors = colors_function(colors = "#332985", sizes = "12", shapes = "circle", borders = NULL) ) %>%
  ...
  label = paste0(
    ...
    <span style='font-weight: bold;'>Council Number:</span> ", council_district_hours$council_num, "</br>",
     ...
     "<span style='font-weight: bold;'>Council District Representative:</span> ", council_district_hours$representat)), "</br></br>",
      ...
      "<span style='font-weight: bold;'>Total Hours spent on Officer-Initiated Stops:</span> ",  scales::comma(council_district_hours$stop_time_cap, accuracy = 1)
...
#palette purples
pal_stop_hours <- colorQuantile("Blues",council_district_hours$stop_time_cap, n = 5)
...
leaflet(width = "100%", height = "495px") %>%
  ...
  addPolygons(data =  council_district_hours,  label=~htmlEscape(paste0("Council Number ", council_num)), color = "#444444", weight = 1, smoothFactor = 0.5, opacity = 1, fillOpacity = 2, popup = ~paste0(stop_hours_popup),
              fillColor = ~pal_stop_hours(stop_time_cap), highlight = highlightOptions(color = "white", weight = 3, bringToFront = TRUE, sendToBack = FALSE), group = "") %>%
 
  ## add legend for stop hours
  addLegend(position = "bottomleft", pal = pal_stop_hours, values = council_district_hours$stop_time_cap, opacity = .5, title = "Total Hours Spent on a Stop", na.label = NA, labFormat = function(type, cuts, p) {
                                      n = length(cuts)
...
high_injury_motor_intersections_spdf$address, "</span>. This intersection ranks Number <span style='font-size: 12px; font-weight: bold;'> ", high_injury_motor_intersections_spdf$rank, "</span> out of 20 High-Injury Motor Vehicle and Motorcycle Intersections.")))
...
high_injury_motor_corridors_line_sf$corridor, "</span>. This corridor runs from <span style='font-size: 12px; font-weight: bold;'>", high_injury_ped_corridors_line_sf$address_1, "</span> to <span style='font-size: 12px; font-weight: bold;'>", high_injury_ped_corridors_line_sf$address_2, ".This corridor ranks Number <span style='font-size: 12px; font-weight: bold;'> ", high_injury_ped_corridors_line_sf$rank, "</span> out of 20 High-Injury Pedestrian and Bicycle Corridors.")))
...
high_injury_motor_corridors_line_sf$corridor, "</span>. This corridor runs from <span style='font-size: 12px; font-weight: bold;'>", high_injury_motor_corridors_line_sf$address_1, "</span> to <span style='font-size: 12px; font-weight: bold;'>", high_injury_motor_corridors_line_sf$address_2, ".This corridor ranks Number <span style='font-size: 12px; font-weight: bold;'> ", high_injury_motor_corridors_line_sf$rank, "</span> out of 20 High-Injury Motor Vehicle and Motorcycle Corridors.")))
...
high_injury_ped_intersections_spdf$address, "</span>. This intersection ranks Number <span style='font-size: 12px; font-weight: bold;'> ", high_injury_ped_intersections_spdf$rank, "</span> out of 20 High-Injury Pedestrian and Bicycle Crashes in Long Beach.")))
...
#palette purples
pal_stop <- colorQuantile("Blues", council_district_rate$total_stop_rate, n = 5)
...
leaflet(width = "100%", height = "495px") %>%
  ...
  # add Long beach district layer with stop rates
  addPolygons(data = council_district_rate,  label=~htmlEscape(paste0("Council Number ", council_num)), color = "#444444", weight = 1, smoothFactor = 0.5,  opacity = 1, fillOpacity = 2,  fillColor = ~pal_stop(total_stop_rate), highlight = highlightOptions(color = "white", weight = 3, bringToFront = FALSE), group = "Council Districts") %>%
 ...
 addCircleMarkers(data = high_injury_ped_intersections_spdf, lat = ~lat, lng = ~lon,
                   group="Pedestrian/Bike",
                popup= ~paste0(high_injury_ped_intersections_popup),
                  label= ~htmlEscape(paste0("High Injury Pedestrian/Bike Intersections")),
                   weight = 2, fillOpacity = 1, radius=3,
                   color="black",  fillColor= "red", stroke = TRUE
                   ) %>%
  
  ## add motor corridors 
   addPolylines(data = high_injury_motor_corridors_line_sf, group = "Motor",
                popup= ~paste0(high_injury_motor_corridors_popup),
                  label= ~htmlEscape(paste0("High Injury Motor Vehicle/Motorcycle Corridors")),
                color = "yellow", weight = 3, opacity = 2) %>%
  
  ## add pedestrian/bike corridors
   addPolylines(data = high_injury_ped_corridors_line_sf, group = "Pedestrian/Bike",
               popup= ~paste0(high_injury_ped_corridors_popup),
                  label= ~htmlEscape(paste0("High Injury Pedestrian/Bike Corridors")),
...
df=report_hitrates_race_person, #name of dataframe
...
chart_title= paste0("Number ofpr_merg Unnecessary searches among traffic stops by Race and Ethnicity"), # this is the more standard chart title that gets used as a subtitle
