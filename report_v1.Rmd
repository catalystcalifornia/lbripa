...
### Officer-initiated stops are not preventing serious threats or crimes {.tabset}
...
hc<-hchart(
 ...
 align="left" )%>%
  ...
  hc_exporting(
    ...
    filename = paste0("LBPD spends most of its time on traffic stops","_Catalyst California, catalystcalifornia.org, 2023.")
  )%>%
...
df<-dbGetQuery(con, "SELECT * FROM report_rates_citations_race_person")
...
mutate(across(everything(), as.character))%>%
  ...
  pivot_longer(!race, names_to="measure", values_to="value")

df_tot<-df_tot%>%
  mutate(Percent=ifelse(measure %in% "rank_1_label", 27.7097621947311,
                     ifelse(measure %in% "rank_2_label", 7.84565091981283,
                            ifelse(measure %in% "rank_3_label", 5.64066405999615,
                                   ifelse(measure %in% "rank_4_label", 5.03172873533748,
                                           ifelse(measure %in% "rank_5_label",4.98685981667842, 'NULL'
                     ))))))%>%
  
    mutate(Total=ifelse(measure %in% "rank_1_label", 4323,
                     ifelse(measure %in% "rank_2_label", 1224,
                            ifelse(measure %in% "rank_3_label", 880,
                                   ifelse(measure %in% "rank_4_label", 785,
                                           ifelse(measure %in% "rank_5_label",778, 'NULL'
                     )))))) %>%
...
A pretextual stop is defined as...  When focusing in on just traffic stops that resulted in no action, individuals who were perceived as having two or more races, or as Black, had the highest rate of traffic stops with no resulting action. This is indicative of a pretextual stop, because for there to be no resulting action to the stop it is likely that there was no serious threat or danger in the first place. (placeholder text)
...
demographic_popup <- paste0((
  ...
  "</span> <br> For every 1K  <span style='font-size: 14px; font-weight: bold;'> Native Hawaiian or Pacific Islander </span> people, LBPD stopped <span style='font-size: 14px; font-weight: bold;'>",
    council_district_demographics$nhpi_stop_rate, "</span> people who identify as Native Hawaiian or Pacific Islander. <span style='font-size: 14px; font-weight: bold;'>",
